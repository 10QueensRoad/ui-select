 ...
 * Version: 0.8.3 - 2014-12-15T23:14:40.335Z
 * License: MIT
 */
...
$templateCache.put("bootstrap/match-multiple.tpl.html","<span class=\"ui-select-match\" id=\"{{$select.id}}-matches\"><span ng-if=\"$select.selected.length\" class=\"sr-only\">currently selected,</span> <span ng-repeat=\"$item in $select.selected\"><span style=\"margin-right: 3px;\" class=\"ui-select-match-item btn btn-default btn-xs\" tabindex=\"-1\" type=\"button\" ng-disabled=\"$select.disabled\" ng-click=\"$select.activeMatchIndex = $index;\" ng-class=\"{\'btn-primary\':$select.activeMatchIndex === $index}\"><span class=\"close ui-select-match-close glyphicon glyphicon-remove\" ng-hide=\"$select.disabled\" aria-hidden=\"true\" ng-click=\"$select.removeChoice($index)\"></span> <span uis-transclude-append=\"\"></span> <span class=\"sr-only\">,</span></span></span></span>");
...
$templateCache.put("bootstrap/select-multiple.tpl.html","<div class=\"ui-select-multiple ui-select-bootstrap dropdown form-control\" ng-class=\"{open: $select.open}\"><div><div class=\"ui-select-match\"></div><input type=\"text\" autocomplete=\"off\" autocorrect=\"off\" autocapitalize=\"off\" spellcheck=\"false\" class=\"ui-select-search input-xs\" placeholder=\"{{$select.getPlaceholder()}}\" ng-disabled=\"$select.disabled\" ng-hide=\"$select.disabled\" ng-click=\"$select.activate()\" ng-model=\"$select.search\" id=\"{{$select.id}}\" aria-describedby=\"{{$select.id}}-matches\" name=\"{{$select.attrs.name}}\"> <span class=\"sr-only\" aria-live=\"polite\" aria-atomic=\"true\">{{$select.currentItemText() ? \'select \' + $select.currentItemText() : \'\'}}</span></div><div class=\"ui-select-choices\"></div></div>");
